				  ...
				  struct thread *thread)
{
	...
	printf_nsecs(tr->dt_between, 6);
	printf_nsecs(tr->dt_delay, 6);
	printf_nsecs(tr->dt_run, 6);
			   ...
			   struct machine *machine)
{
	...
	while (iter) {
		...
		if (node->sym && symbol__is_idle(node->sym->name))
			return true;
				 ...
				 struct thread_runtime *r)
{
	...
	printf_nsecs(r->total_run_time, 8);
	stddev = rel_stddev_stats(stddev_stats(&r->run_stats), mean);
	printf_nsecs(r->run_stats.min, 6);
	printf(" ");
	printf_nsecs((u64) mean, 6);
	printf(" ");
				   ...
				   struct perf_session *session)
{
	...
	for (i = 0; i <= idle_max_cpu; ++i) {
		...
		if (r && r->run_stats.n) {
			...
			printf_nsecs(r->total_run_time, 6);
			printf(" msec\n");
		} else
	...
	printf_nsecs(totals.total_run_time, 2);
	printf("\n");
}
...
static __attribute__((used)) int timehist_symbol_filter(struct map *map, struct symbol *sym)
{
